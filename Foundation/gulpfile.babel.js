...
gulp.task('build',
  gulp.series(clean, pages, creds, moveCSS, images, inline));
...
function watch() {
  ...
  gulp.watch(['../scss/**/*.scss', 'src/assets/scss/**/*.scss']).on('all', gulp.series(resetPages, pages,moveCSS, inline, browser.reload));
...
function moveCSS(){
  ...
  console.log(awsURL);
  gulp.src("dist/**/*.html")
  .pipe(rep("/assets/img", awsURL))
  .pipe(gulp.dest("dist"));

  return gulp.src("src/assets/scss/app.css")
  .pipe(gulp.dest("dist/css"));
}